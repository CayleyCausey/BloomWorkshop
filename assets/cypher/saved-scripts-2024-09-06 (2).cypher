...
//Load Metro
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/CayleyCausey/BloomWorkshop/main/assets/data/dcmetro/stations.csv' as row
MERGE (s:Station {name:row.Station}) 
WITH row,COLLECT(s) AS station
CALL apoc.create.addLabels(station,[row.Line]) YIELD node
RETURN node;

// Loading geos
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/CayleyCausey/BloomWorkshop/main/assets/data/dcmetro/geos.csv' AS row
MATCH (s:Station) WHERE s.name = row.STATION
SET s.lat = toFloat(row.Lat), s.lon = toFloat(row.Lon)
RETURN s.lat,s.lon;

// Loading lines
LOAD CSV FROM 'https://raw.githubusercontent.com/CayleyCausey/BloomWorkshop/main/assets/data/dcmetro/lines.csv' as row
MATCH (s1:Station) WHERE s1.name = row[0]
MATCH (s2:Station) WHERE s2.name = row[2]
WITH s1, s2, toUpper(row[1]) as Line
CALL apoc.merge.relationship(s1,Line,{},{},s2) YIELD rel RETURN rel
...
MATCH (a)-[:ACTED_IN]->(m:Movie) SET a:Actor;
MATCH (a)-[:DIRECTED]->(m:Movie) SET a:Director;
MATCH (a)-[:PRODUCED]->(m:Movie) SET a:Producer;
MATCH (a)-[:WROTE]->(m:Movie) SET a:Writer;
